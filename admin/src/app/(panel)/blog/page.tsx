...
import { api, BlogPost, getToken } from "@/lib/api";
...
export default function BlogPage() {
  const { data: posts, reload: load } = useCached("admin/blog", () =>
  ...
  async function save(e: React.FormEvent) {
    ...
    try {
      ...
      if (form.id) await api.patch(`/admin/blog/${form.id}`, payload);
    ...
    <div>
            ...
            : posts.map((p, i) => (
                ...
                >
                  <Card className="flex items-center gap-3 p-3">
                    <div className="relative flex h-16 w-16 shrink-0 items-center justify-center overflow-hidden rounded-xl bg-panel-2 text-muted">
                      {p.imageUrl ? (
                        <Image
                          src={p.imageUrl}
                          alt={p.title}
      ...
      >
        <p className="text-sm text-muted">
          Delete{" "}
          <span className="font-medium text-fg">{confirmDel?.title}</span>? This
          cannot be undone.
        </p>
